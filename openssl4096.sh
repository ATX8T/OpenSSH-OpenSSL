...
# ================================================
# OpenSSL 自签名证书生成脚本（4096位RSA增强版）
# 生成时间：2025-04-07 02:35 
# 版本：v3.1.4096 
# ================================================
...
verify_certificate() {
    ...
    # 密钥强度验证 
    echo -e "\n\e[36m[强度检测]\e[0m"
    local key_info=$(openssl rsa -in "$cert_dir/server.key"  -text -noout 2>/dev/null)
    ...
    # 有效期验证 
    echo -e "\n\e[36m[有效期]\e[0m"
    openssl x509 -in "$cert_dir/server.crt"  -noout -dates | awk -F= '
...
handle_directory_conflict() {
    echo -e "\n\e[33m[警告] 目录中存在旧证书文件！\e[0m"
    ls -lh "$1"/server.* 2>/dev/null 
...
main() {
    ...
    # 冲突检测 
    if ls "$cert_dir"/server.* >/dev/null 2>&1; then 
    ...
    # 最终报告 
    echo -e "\n\e[1;35m=== 生成结果 ===\e[0m"
    echo -e "\e[33m私钥路径：\e[0m$(realpath "$cert_dir/server.key")" 
    echo -e "\e[33m证书路径：\e[0m$(realpath "$cert_dir/server.crt")" 
    echo -e "\e[33m公钥路径：\e[0m$(realpath "$cert_dir/server.pub")" 
